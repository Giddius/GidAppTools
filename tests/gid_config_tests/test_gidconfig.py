...
result_item_1 = ResultItem(spec_name="basic",
                           config_name="basic",
                           get_checks={("first_section", "entry_one"): "one",
...
def test_complete_gid_config(in_spec_path: Path, in_config_path: Path, preload_ini: bool, result_item: ResultItem):
    ...
    assert config.spec.name == result_item["spec_name"]
    assert config.config.name == result_item["config_name"]
